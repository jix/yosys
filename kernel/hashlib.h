...
public:
	...
	// call. Consider the atomic writes performed by various ifind() calls, in order.
	// The first atomic write can only set parents[k] to R, because the atomic read
	// of parents[p] in the first while loop can only observe the value 'orig_parents[p]'.
	// Subsequent writes can also only set parents[k] to R, because the parents[p] reads
