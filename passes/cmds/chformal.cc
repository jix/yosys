...
struct ChformalPass : public Pass {
	...
	void help() override
	{
		...
		log("\n");
		log("    Additionally chformal will operator on $check cells corresponding to the\n");
		...
		for (auto module : design->selected_modules())
		{
				...
				for (auto cell : constr_cells)
				{
					if (cell->type == ID($check)) {
						...
						for (auto conn : cell->connections())
							cover->setPort(conn.first, conn.first == ID::A ? State::S1 : conn.second);
					} else {
			...
			if (mode == 'l')
			{
				for (auto cell : constr_cells) {
					...
					SigBit id_a = cell->getPort(ID::A);
					SigBit id_en = cell->getPort(ID::EN);

					plain_cell->setPort(ID::A, id_a);
					plain_cell->setPort(ID::EN, id_en);

					SigBit combined_en = module->And(NEW_ID, id_a, id_en);
